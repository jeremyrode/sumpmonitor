...
const ZERO_CURRENT_CODE = 0;
const CURRENT_SLOPE = 1;
...
ADS1115.open(0, 0x48).then(async (ads1115) => {
  ...
  while (true) {
    ...
    if (cur_current > 32768) { // 2's compliment
    ...
    let current_amps = (ave_current - ZERO_CURRENT_CODE) / CURRENT_SLOPE;
    const curDate = new Date();
    const strcurDate = curDate.toString();
    if (mesaurementCount >= DATA_CAPTURE_INTERVAL) { //Take a datapoint 
      measurmentArray.push([(curDate - dateOffset) / dayFraction, depth_inches.toFixed(3), ave_current.toFixed(3)]);
      datafile.write(curDate.valueOf() + ',' + depth_inches.toFixed(3) + ',' + ave_current.toFixed(3) + '\n');
      mesaurementCount = 0;
    }
    ...
    //lcd.printLineSync(2, depth_inches.toFixed(2).toString().padStart(6) + ' ' + cur_current.toString());
    lcd.printLineSync(2, depth_inches.toFixed(2).toString().padStart(6) + cur_current.toString().padStart(6));
    mesaurementCount += 1;
